...
function Carousel() {
  ...
  async function fetchTopics() {
    try {
      ...
      const generalKnowledgeTopics = data.filter(item => item.category === "General Knowledge");
      
      setTopics(generalKnowledgeTopics);
    } catch (error) {
  ...
  return (
    <div className="group relative bg-slate-950">
      <h1 className="text-center pt-8 text-2xl font-bold text-white light:text-white">General Knowledge</h1>
      <div className='flex items-center justify-center my-1'>
        <button
          className="absolute w-8 h-40 left-0 z-10 m-4 opacity-0 group-hover:opacity-100 hover:bg-red-300 transition-opacity duration-300"
          onClick={handlePrev}
        ...
        <button
          className="right-0 z-0 w-8 h-40 opacity-0 group-hover:opacity-100 hover:bg-red-300 transition-opacity duration-300"
          onClick={handleNext}
