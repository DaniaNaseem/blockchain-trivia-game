...
import GameOver from './GameOverPractice';

export default function PracticeQuestionScreen({ socket }) {
  ...
  const handleUser1Answer = (selectedAnswer) => {
    ...
    if (!user1Answer && !clockEnded && roomId) {
      ...
      socket.emit('optionSelectedPractice', { selectedOption: selectedAnswer, room_id: roomId });
  ...
  useEffect(() => {
    ...
    socket.on('roundStartPractice', (data) => {
      setAnswerData(null);
    ...
    return () => {
      socket.off('roundStartPractice');
      // Add more event listeners if needed
    };
  }, [socket]);

  useEffect(() => {
    socket.on('answerCheckedPractice', (data) => {
    ...
    return () => {
      socket.off('answerCheckedPractice');
      // Add more event listeners if needed
    };
  }, [socket]);

  useEffect(() => {
    socket.on('gameOverPractice', (data) => {
  ...
  useEffect(() => {
    socket.on('prepareForNextRoundPractice', (data) => {
      ...
      if (data && data.round) {
        ...
        setTimeout(() => {
          ...
          if (data && data.room_id && data.round) {
            ...
            socket.emit('pleaseStartNextRoundPractice', { room_id: data.room_id, round: data.round });
    ...
    return () => {
      socket.off('prepareForNextRoundPractice');
    };
  }, [socket]);


  useEffect(()=>{
    socket.on('timerEventPractice', (data) => {
